locals {
  dcs_principal_role_name   = "DcsPrincipal${var.namespace == "prod" ? "" : "-${var.namespace}"}"
  dcs_principal_policy_name = "DcsPrincipalDefaultPolicy${var.namespace == "prod" ? "" : "-${var.namespace}"}"
}
...
resource "aws_sns_topic" "account_created" {
  name = "dcs-account-created-${var.namespace}"
  tags = var.global_tags
}

resource "aws_sns_topic" "account_deleted" {
  name = "dcs-account-deleted-${var.namespace}"
  tags = var.global_tags
}

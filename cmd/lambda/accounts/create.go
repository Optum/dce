...
type createController struct {
	...
	// The IAM Dcs Principal role will be denied access
...
func (c createController) Call(ctx context.Context, req *events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	if err != nil {
		log.Printf("Failed to add dcs account %s to pool: %s",
	...
	_, err = c.TokenService.AssumeRole(&sts.AssumeRoleInput{
		...
		RoleSessionName: aws.String("DcsMasterAssumeRoleVerification"),
	})
	if err != nil {
		return response.RequestValidationError(
			"Unable to create Account: adminRole is not assumable by the Dcs master account",
		), nil
	}
	...
	// Create an IAM Role for the Dcs principal (end-user) to login to

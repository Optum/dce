...
func (controller getAccountByIDController) Call(ctx context.Context, req *events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	// Fetch the account.
	...
	if err != nil {
		...
		return response.CreateAPIErrorResponse(http.StatusInternalServerError,
			response.CreateErrorResponse("ServerError",
				fmt.Sprintf("Failed List on Account Assignment %s",
					acctID))), nil
	}
	if account == nil {
		log.Printf("Error Getting Account for AccountId: %s", err)

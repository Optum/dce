	...
	t.Run("Account Creation Deletion Flow", func(t *testing.T) {
		...
		t.Run("STEP: Create Account", func(t *testing.T) {
			...
			t.Run("STEP: Create Lease", func(t *testing.T) {
				...
				t.Run("STEP: Lease Auth", func(t *testing.T) {
					// Attempt to authenticate into the lease
					res = apiRequest(t, &apiRequestInput{
						method:      "POST",
						maxAttempts: 3,
					...
					leaseCreds := credentials.NewStaticCredentials(
						credsJSON["accessKeyId"].(string),
						credsJSON["secretAccessKey"].(string),
						credsJSON["sessionToken"].(string),
					)
					_ = apiRequest(t, &apiRequestInput{
						...
						url:         apiURL + "/accounts",
						f: func(r *testutil.R, apiResp *apiResponse) {
							assert.Equalf(r, 200, apiResp.StatusCode, "%v", apiResp.json)
						},
					})

					// Console URL should redirect us to console.aws.amazon.com
					...
					res, err := httpClient.Get(credsJSON["consoleUrl"].(string))
					require.Nil(t, err)
					require.Equal(t, 302, res.StatusCode,
						"Console URL should redirect (HTTP 302)")
					require.Equal(t, "https://console.aws.amazon.com/", res.Header.Get("Location"),
						"Console URL should redirect to AWS web console")

					t.Run("should 404 if the lease doesn't exist", func(t *testing.T) {
						res := apiRequest(t, &apiRequestInput{
							method:      "POST",
							maxAttempts: 3,
							url:         fmt.Sprintf("%s/leases/not-a-lease/auth", apiURL),
							f: func(r *testutil.R, apiResp *apiResponse) {
								assert.Equal(r, 404, apiResp.StatusCode)
							},
						})
						resJSON = parseResponseJSON(t, res)
						...
						require.Equal(t, map[string]interface{}{
							"error": map[string]interface{}{
								"message": "lease \"not-a-lease\" not found",
								"code":    "NotFoundError",
							},
						}, resJSON)
					})

					// TODO: test implicit lease ID as Authenticated user (ie. with Cognito)
					t.Run("should fail on implicit lease ID for admin user", func(t *testing.T) {
						res := apiRequest(t, &apiRequestInput{
							method:      "POST",
							maxAttempts: 3,
							url:         fmt.Sprintf("%s/leases/auth", apiURL),
							f: func(r *testutil.R, apiResp *apiResponse) {
								assert.Equal(r, 404, apiResp.StatusCode)
							},
						})
						...
						require.Equal(t, map[string]interface{}{
							"error": map[string]interface{}{
								"message": "lease for user \"\" not found",
								"code":    "NotFoundError",
							},
						}, resJSON)
					})

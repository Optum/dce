...
# The premier repository for AWS Dce.
...
- [Implementing Dce](#implementing-dce)
  * [Deploying Dce](#deploying-dce)
  * [Using the Dce API](#using-the-dce-api)
  * [Integrating with Dce](#integrating-with-dce)
  * [Integrating with Identity Providers](#integrating-with-identity-providers)
  * [Configuring aws-nuke](#configuring-aws-nuke)
- [Usage](#usage)
  * [Adding AWS Accounts to the Dce Account Pool](#adding-aws-accounts-to-the-dce-account-pool)
  * [Authenticating into Dce Accounts](#authenticating-into-dce-accounts)
...
This repo provides a set of components which implementors (you) can use to deploy your own Dce instance. These components come in the form of:
...
When a new AWS account is added to the pool, [a role is created to allow principal users to login to the account](#adding-aws-accounts-to-the-dce-account-pool), designated by the `adminRoleArn` field on the account object. By default, this role has an Assume Role Policy allowing IAM principals from the same account to assume it.
...
The Terraform module will come with a sane starting policy that is applied to the IAM principal.  This policy is applied when a new account is added or when a lease is unlocked.  It is possible to change the policy to what is needed by providing the Terraform variable `dce_principal_policy`.  The value of this variable is a location of a policy file that can be a Go template.  It is uploaded into S3 and is read from there as the policy is applied.
...
table_name=$(cd modules && terraform output dce_account_db_table_name)

# Or, grab the leases table name
table_name=$(cd modules && terraform output dce_lease_db_table_name)
...
Dce API Spec available via swaggerUI on github host: [API Spec](https://github.optum.com/pages/CommercialCloud-Team/aws_dce/)

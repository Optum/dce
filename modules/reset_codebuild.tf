 ...
 * execute our Dcs Account Reset process.
...
resource "aws_codebuild_project" "reset_build" {
  name          = "dcs-reset-${var.namespace}"
  description   = "Execute Dcs Account reset for an AWS Account"
  build_timeout = "480"
  ...
  environment {
    ...
    environment_variable {
      name  = "ACCOUNT_DB"
      value = aws_dynamodb_table.dcs_account.id
      type  = "PLAINTEXT"
    }

    environment_variable {
      name  = "LEASE_DB"
      value = aws_dynamodb_table.dcs_lease.id
      type  = "PLAINTEXT"
    }
...
resource "aws_iam_role" "codebuild_reset" {
  name = "dcs-reset-codebuild-${var.namespace}"
...
resource "aws_sns_topic" "reset_complete" {
  name = "dcs-reset-complete-${var.namespace}"
  tags = var.global_tags
}

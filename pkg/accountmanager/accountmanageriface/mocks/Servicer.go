...
// Credentials provides a mock function with given fields: role, roleSessionName, duration
func (_m *Servicer) Credentials(role *arn.ARN, roleSessionName string, duration *time.Duration) accountmanageriface.Credentialer {
	ret := _m.Called(role, roleSessionName, duration)

	var r0 accountmanageriface.Credentialer
	if rf, ok := ret.Get(0).(func(*arn.ARN, string, *time.Duration) accountmanageriface.Credentialer); ok {
		r0 = rf(role, roleSessionName, duration)
	} else {
		if ret.Get(0) != nil {

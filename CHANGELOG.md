...
This release removes a number of Optum-specific configurations from the default aws-nuke YAML configuration. If you want to keep these configurations in your implementation of Dcs, you will need to specify an _override_ nuke config as part of Optum's deployment of Dcs.
...
- Added EKS services to allowed services in policy file, dcsprincipal.go
...
- Add SES terraform to enable email from Dcs App for notifications.
...
- Remove `DcsAccountAssignment` DyanmoDB table
  - This table was deprecated in v0.10.0, and no longer referenced in AWS Dcs code
...
- Rename `Principal` --> `User`; `Assignment` --> `Lease`. Includes:
  - Create new `DcsLease` table, migrate data from `DcsAccountAssignment` table
    - Note that in this release, both tables exist in order to allow for migrations. The `DcsAccountAssignment` is deprecated, and will destroyed in a subsequent release.
...
- Remove `pkg/authorization` and `pkg/common/jwt*` as they are no longer
  integrated with the base aws_dcs.
...
- Update `data "template_file" "aws_dcs_api_swagger"` to use the relative
  path of the Terraform module to read the `swaggerDcs.yaml` file.
...
- Split apart open-source aws_dcs from Optum-specific implementation:

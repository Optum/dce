...
resource "aws_api_gateway_deployment" "gateway_deployment_dcs" {
  rest_api_id = aws_api_gateway_rest_api.gateway_api.id

  stage_name = "dcs-${var.namespace}"
  ...
  variables = {
    ...
    change_trigger = sha256(data.template_file.aws_dcs_api_swagger.rendered)
  }
...
// IAM principals to talk to Dcs APIs
resource "aws_iam_policy" "api_execute_admin" {
  name        = "dcs-api-execute-admin-${var.namespace}"
  description = "Provides access to all Dcs admin API endpoints"

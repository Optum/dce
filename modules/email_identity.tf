...
resource "aws_ses_email_identity" "dce_from_email_address" {
  email = var.budget_notification_from_email
}

# Send delivery failures and bounces to SNS topic
resource "aws_ses_configuration_set" "dce_ses" {
  name = "dce_ses_${var.namespace}"
}

resource "aws_ses_event_destination" "dce_ses_cloudwatch" {
  name                   = "event-destination-cloudwatch-${var.namespace}"
  configuration_set_name = "${aws_ses_configuration_set.dce_ses.name}"
  enabled                = true

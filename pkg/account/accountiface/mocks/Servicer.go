...
// GetUsageBetweenDates provides a mock function with given fields: data, startDate, endDate
func (_m *Servicer) GetUsageBetweenDates(data *account.Account, startDate time.Time, endDate time.Time) (types.Usages, error) {
	ret := _m.Called(data, startDate, endDate)

	var r0 types.Usages
	if rf, ok := ret.Get(0).(func(*account.Account, time.Time, time.Time) types.Usages); ok {
		r0 = rf(data, startDate, endDate)
	} else {
		if ret.Get(0) != nil {

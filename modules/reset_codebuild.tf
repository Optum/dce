...
resource "aws_codebuild_project" "reset_build" {
  name          = "${var.namespace_prefix}-reset-${var.namespace}"
  description   = "Execute Account reset for an AWS Account"
  build_timeout = "480"
...
resource "aws_iam_role" "codebuild_reset" {
  name = "${var.namespace_prefix}-reset-codebuild-${var.namespace}"
...
resource "aws_sns_topic" "reset_complete" {
  name = "${var.namespace_prefix}-reset-complete-${var.namespace}"
  tags = var.global_tags
}

...
type DB struct {
	...
	// Name of the DcsAccount table
	AccountTableName string
	// Name of the DcsLease table
...
type DBer interface {
	GetAccount(accountID string) (*DcsAccount, error)
	GetReadyAccount() (*DcsAccount, error)
	GetAccountsForReset() ([]*DcsAccount, error)
	GetAccounts() ([]*DcsAccount, error)
	FindAccountsByStatus(status AccountStatus) ([]*DcsAccount, error)
	FindAccountsByPrincipalID(principalID string) ([]*DcsAccount, error)
	PutAccount(account DcsAccount) error
	DeleteAccount(accountID string) (*DcsAccount, error)
	PutLease(account DcsLease) (*DcsLease, error)
	TransitionAccountStatus(accountID string, prevStatus AccountStatus, nextStatus AccountStatus) (*DcsAccount, error)
	TransitionLeaseStatus(accountID string, principalID string, prevStatus LeaseStatus, nextStatus LeaseStatus) (*DcsLease, error)
	FindLeasesByAccount(accountID string) ([]*DcsLease, error)
	FindLeasesByPrincipal(principalID string) ([]*DcsLease, error)
	FindLeasesByStatus(status LeaseStatus) ([]*DcsLease, error)
	UpdateMetadata(accountID string, metadata map[string]interface{}) error
	UpdateAccountPrincipalPolicyHash(accountID string, prevHash string, nextHash string) (*DcsAccount, error)
}

// GetAccount returns a Dcs account record corresponding to an accountID
// string.
func (db *DB) GetAccount(accountID string) (*DcsAccount, error) {
...
func (db *DB) GetAccounts() ([]*DcsAccount, error) {
	...
	if err != nil {
		return make([]*DcsAccount, 0), err
	}

	// Return the Dcs Account
	accounts := []*DcsAccount{}
...
// GetReadyAccount returns an available Dcs account record with a
// corresponding status of 'Ready'
func (db *DB) GetReadyAccount() (*DcsAccount, error) {
...
// GetAccountsForReset returns an array of Dcs account records available to
// be Reset
func (db *DB) GetAccountsForReset() ([]*DcsAccount, error) {
	...
	// Create the array of DcsAccounts
	dcses := []*DcsAccount{}
	for _, r := range resp.Items {
		n, err := unmarshalAccount(r)
		if err != nil {
		...
		dcses = append(dcses, n)
	}
	return dcses, nil
}

func (db *DB) FindAccountsByStatus(status AccountStatus) ([]*DcsAccount, error) {
...
func (db *DB) FindAccountsByPrincipalID(principalID string) ([]*DcsAccount, error) {
	res, err := db.Client.Query(&dynamodb.QueryInput{
		TableName: aws.String(db.AccountTableName),
		ExpressionAttributeValues: map[string]*dynamodb.AttributeValue{
			":pid": {
...
func (db *DB) GetLease(accountID string, principalID string) (*DcsLease, error) {
	result, err := db.Client.GetItem(
		&dynamodb.GetItemInput{
...
func (db *DB) FindLeasesByAccount(accountID string) ([]*DcsLease, error) {
...
func (db *DB) FindLeasesByPrincipal(principalID string) ([]*DcsLease, error) {
	input := &dynamodb.QueryInput{
...
func (db *DB) FindLeasesByStatus(status LeaseStatus) ([]*DcsLease, error) {
	res, err := db.Client.Query(&dynamodb.QueryInput{
...
// PutAccount stores a Dcs account in DynamoDB
func (db *DB) PutAccount(account DcsAccount) error {
...
func (db *DB) PutLease(lease DcsLease) (
	*DcsLease, error) {
...
func (db *DB) TransitionLeaseStatus(accountID string, principalID string, prevStatus LeaseStatus, nextStatus LeaseStatus) (*DcsLease, error) {
...
func (db *DB) TransitionAccountStatus(accountID string, prevStatus AccountStatus, nextStatus AccountStatus) (*DcsAccount, error) {
	result, err := db.Client.UpdateItem(
		&dynamodb.UpdateItemInput{
			// Query in Lease Table
			TableName: aws.String(db.AccountTableName),
...
func (db *DB) UpdateAccountPrincipalPolicyHash(accountID string, prevHash string, nextHash string) (*DcsAccount, error) {
...
func (db *DB) DeleteAccount(accountID string) (*DcsAccount, error) {
...
func unmarshalAccount(dbResult map[string]*dynamodb.AttributeValue) (*DcsAccount, error) {
	dcsAccount := DcsAccount{}
	err := dynamodbattribute.UnmarshalMap(dbResult, &dcsAccount)

	if err != nil {
		return nil, err
	}

	return &dcsAccount, nil
}

func unmarshalLease(dbResult map[string]*dynamodb.AttributeValue) (*DcsLease, error) {
	dcsLease := DcsLease{}
	err := dynamodbattribute.UnmarshalMap(dbResult, &dcsLease)
	if err != nil {
		return nil, err
	}
